...
describe('App', () => {
  it('renders the seeded portfolio and exactly three holdings', async () => { const wrapper=mount(App,{props:{api:api()}}); await flushPromises(); expect(wrapper.text()).toContain('Interview Demo Portfolio'); expect(wrapper.findAll('[data-holding]')).toHaveLength(3) })
