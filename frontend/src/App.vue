...
<template>
  <main>
    <header><p class="eyebrow">PORTFOLIO COMPLIANCE</p><h1>{{portfolio?.name ?? 'Loading portfolio…'}}</h1><p v-if="portfolio" class="nav">Total NAV <strong>${{portfolio.nav}}</strong></p></header>
    <p v-if="error" role="alert" class="error">{{error}}</p>
    <section v-if="portfolio" aria-labelledby="holdings-title"><h2 id="holdings-title">Current holdings</h2><div class="holdings"><article v-for="holding in portfolio.holdings" :key="holding.id" data-holding><h3>{{holding.name}}</h3><p>${{holding.value}}</p><strong>{{holding.exposure}}</strong></article></div><p class="remainder">{{portfolio.remainder}}</p></section>
    <section v-if="portfolio" class="trade" aria-labelledby="trade-title"><div><p class="eyebrow">PROPOSED BUY</p><h2 id="trade-title">Alpha Global Equity Fund</h2></div><form @submit.prevent="check"><label for="amount">Trade amount (USD)</label><input id="amount" v-model="amount" inputmode="decimal" autocomplete="off"><div class="presets"><button v-for="preset in ['9,999.00','10,000.00','10,001.00']" :key="preset" type="button" @click="amount=preset">${{preset}}</button></div><button class="check" type="submit" :disabled="loading">{{loading?'Checking…':'Check compliance'}}</button></form></section>
